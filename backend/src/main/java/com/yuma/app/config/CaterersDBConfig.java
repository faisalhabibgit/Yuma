...
@Slf4j
@EnableMongoRepositories("com.yuma.app.document")
...
public class CaterersDBConfig {
	...
	CommandLineRunner commandLineRunner(MealRepository mealRepository) {
		...
		return strings -> {
			mealRepository.save(new Meal(ingredientsList, UUID.randomUUID(), "ChickenPlate", true));
			mealRepository.save(new Meal(ingredientsList, UUID.randomUUID(), "Beef and stuff", true));
	...
	CommandLineRunner commandLineRunner(ConsumersRepository consumersRepository, MealRepository mealRepository) {
		logger.info("this is logging Consumers into the DB");
		return strings -> {
